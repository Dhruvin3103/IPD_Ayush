...
const Navbar_2 = dynamic(() => import('@/timepass/Navbar_2'));
const Footer = dynamic(() => import('@/timepass/Footer'));
const Map2 = dynamic(() => import('@/timepass/Map2'));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-12 w-full">
              <div className="flex flex-col justify-center space-y-6 md:space-y-4 lg:space-y-6 md:w-30%">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">
                  Nearest Parking Zones
                </h2>
                {/* Map through the fetched locations and render cards */}
                {locations.map((location, index) => (
                  <div
                    key={index}
                    className="bg-white p-4 rounded-lg shadow-md dark:bg-gray-900"
                  >
                    ...
                    <button onClick={() => handleLocationClick(location)} className="block mt-2 text-center bg-blue-500 hover:bg-red-500 text-white font-bold py-2 px-3 rounded transition-colors duration-300 text-xs md:text-sm">
                      View on Map
                    </button>
                  </div>
                ))}
                  ...
                  <div className="h-screen flex justify-center items-center">
                    ...
                    >
                      ...
                      <div className="mx-auto aspect-video overflow-hidden rounded-xl object-cover object-center w-full md:h-full lg:w-full">
                        <div>
                        <Map2 selectedLocation={selectedLocation} />
                        </div>
                      </div>
                    </div>
                  </div>
